...
class RopeEmbedder:
    ...
    def rope_params(self, index, dim, theta=256.0, device=None):
        """
        计算RoPE参数的基础函数 - 在CPU上计算以避免GPU复数支持问题
        Args:
            index: 位置索引
            dim: 维度
            theta: RoPE的theta值
    ...
    def rope_params_yarn(self, index, dim, theta, current_patches, device,):
        """
        应用YARN插值with DyPE到RoPE参数 - 在CPU上计算以避免GPU复数支持问题
        """
        ...
        # if torch.any(index < 0) and self.dype: # 中心对称坐标实现有问题，暂时忽略
        #     min_val = index.min()
        #     index = index - min_val 
        ...
        #     print(f"Negative indices found: {index[index < 0]}")
        #     print(f"Index range: [{index.min()}, {index.max()}]")
        #     print(f"Scale: {scale}")
        ...
        # 动态调整YARN参数（DyPE）
        if self.dype:
            timestep_val = self.current_timestep.item() if hasattr(self.current_timestep, 'item') else self.current_timestep
            timestep_factor = timestep_val ** 2.0
        ...
        return freqs_complex.to(target_device)

   
    def set_timestep(self, timestep: float):
        """设置当前时间步用于DyPE。时间步归一化到[0,1]范围，其中1表示纯噪声。"""
        if self.dype:
        # 确保 timestep 是标量值
            if isinstance(timestep, torch.Tensor):
                if timestep.numel() == 1:
                    self.current_timestep = timestep.item()
                else:
                    # 如果是多个元素的张量，取其均值作为单一时间步值
                    self.current_timestep = timestep.float().mean().item()
            else:
                self.current_timestep = float(timestep)
        else:
            self.current_timestep = 1.0  # 默认值
    def __call__(self, ids: torch.Tensor):
        """
        针对基于块的RoPE优化的调用函数，保持空间连续性
        """
        assert ids.ndim == 2
        ...
        for i in range(len(self.axes_dims)):
            ...
            if i == 0:  # 文本维度，不需要缩放
                ...
                # 分别处理高度和宽度维度
                current_patches = pos_indices.max().item() + 1  
                if self.dype :  
                    freqs_complex = self.rope_params_yarn(
                        pos_indices, d, self.theta, current_patches, device,
                    )
                else:
                    print(f"Using standard RoPE for dimension {i}")
                    freqs_complex = self.rope_params(pos_indices, d, self.theta, device)
...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        omni_mode = isinstance(x[0], list) and isinstance(cap_feats[0], list)

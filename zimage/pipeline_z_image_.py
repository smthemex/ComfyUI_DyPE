...
class ZImagePipeline(DiffusionPipeline, ZImageLoraLoaderMixin, FromSingleFileMixin):
    ...
    def __init__(
        ...
        self.vae_scale_factor = (
            2 ** (len(self.vae.block_out_channels) - 1) if hasattr(self, "vae") and self.vae is not None else 8
        )
    ...
    def __call__(
        ...
        if negative_prompt_embeds.shape[1] != prompt_embeds.shape[1]:
            negative_prompt_embeds = negative_prompt_embeds.repeat(batch_size, 1)
        ...
        if output_type == "latent":
            ...
            image = (latents / self.vae.scaling_factor )+ self.vae.shift_factor
        else:

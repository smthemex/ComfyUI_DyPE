...
class RopeEmbedder:
    def __init__(
        ...
        self.base_patches = self.base_resolution // 16  
    ...
    def rope_params(self, index, dim, theta=256.0, device=None):
        assert dim % 2 == 0
        if device is None:
        ...
        return freqs
    def set_caption_info(self, caption_length: int, image_start_idx: int):
        """设置caption信息用于区分token类型"""
    ...
    def rope_params_yarn(self, index, dim, theta, current_patches, i,device):

        if not self.dype or current_patches <= self.base_patches:
            return self.rope_params(index, dim, theta, device)
        # if i == 0:
        #     current_patches = self.axes_lens[i]
        # else:
        #     if not self.scale_rope:
        #         current_patches = index.max().item() + 1 
        current_patches = index.max().item() + 1 
        # if self.scale_rope:
        #     if i == 0:
        #         scale = max(1.0, self.axes_lens[i]/2 / self.base_patches)
        #     else:
        #         scale = max(1.0, current_patches / self.base_patches)
        # else:
        #     scale = max(1.0, current_patches / self.base_patches)
        scale = max(1.0, min( self.h_patches, self.w_patches) / self.base_patches)
        if not isinstance(index, torch.Tensor):
            index = torch.tensor(index, device=device, dtype=torch.float32)
        else:
            index = index.to(device).float()

        if self.scale_rope and i>0: 
            index=index+0.75*current_patches
        ...
        if self.dype:
            timestep_factor = min(2.0, self.current_timestep ** 2.0)
            beta_0 = min(2.0, beta_0 ** (2.0 * timestep_factor))
            beta_1 = max(0.1, beta_1 ** (2.0 * timestep_factor))
            gamma_0 = min(32.0, gamma_0 ** (2.0 * timestep_factor))
            gamma_1 = max(0.1, gamma_1 ** (2.0 * timestep_factor))
    ...
    def __call__(self, ids: torch.Tensor):
        ...
        device = ids.device
        self.axes_lens=[max( 64,self.h_patches , self.w_patches) ,self.h_patches, self.w_patches]
        ...
        for i in range(len(self.axes_dims)):
            ...
            pos_indices = ids[:, i]    
            if not self.dype:  
                freqs_complex = self.rope_params(pos_indices, d, self.theta, device)
            else: 
                freqs_complex = self.rope_params_yarn(
                    pos_indices, d, self.theta, e, i,device
                )
...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def patchify_and_embed(
        ...
        for i, (image, cap_feat) in enumerate(zip(all_image, all_cap_feats)):
            ...
            cap_padded_pos_ids_3d = torch.zeros((cap_padded_pos_ids.shape[0], 3), dtype=torch.int32, device=device)

            cap_padded_pos_ids_3d[:, 0] = cap_padded_pos_ids[:, 0]  
            all_cap_pos_ids.append(cap_padded_pos_ids_3d)
            ...
            f_pos = torch.arange(F_tokens, device=device)
            if self.dype and self.scale_rope:
                h_pos = torch.cat([
                    torch.arange(-(H_tokens - H_tokens // 2), 0, device=device),
                    torch.arange(0, H_tokens // 2, device=device)
                ])
                w_pos = torch.cat([
                    torch.arange(-(W_tokens - W_tokens // 2), 0, device=device),
                    torch.arange(0, W_tokens // 2, device=device)
                ])
            else:

                h_pos = torch.arange(H_tokens, device=device)
                w_pos = torch.arange(W_tokens, device=device)

            f_grid, h_grid, w_grid = torch.meshgrid(f_pos, h_pos, w_pos, indexing='ij')

            image_ori_pos_ids_3d = torch.stack([
                f_grid + (cap_ori_len + cap_padding_len + 1),  
                h_grid, 
                w_grid   
            ], dim=-1).reshape(-1, 3)

            # time_start_offset = cap_ori_len + cap_padding_len + 1
            # self.rope_embedder.set_time_offset(F_tokens)
            image_start_idx = cap_ori_len + cap_padding_len + 1
            self.rope_embedder.set_caption_info(cap_ori_len, image_start_idx)
            print(f"Batch {i}: cap_ori_len={cap_ori_len}, cap_padding_len={cap_padding_len}, image_start_idx={image_start_idx}",image_ori_len)
            print(f"H_tokens={H_tokens}, W_tokens={W_tokens}",F_tokens)

            image_padded_pos_ids_3d = torch.cat(
                [
                    image_ori_pos_ids_3d,
                    torch.zeros((image_padding_len, 3), dtype=torch.int32, device=device),
                ],
                dim=0,
            )
            all_image_pos_ids.append(image_padded_pos_ids_3d if image_padding_len > 0 else image_ori_pos_ids_3d)

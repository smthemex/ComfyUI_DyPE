...
class Flux2PosEmbed(nn.Module):
    ...
    def forward(self, ids: torch.Tensor,is_txt=True) -> torch.Tensor: ##B, text_seq_len, 4 # B, image_seq_len, 4
        ...
        for i in range(n_axes):
            ...
            current_patches=min(pos[:, 2].max().item()+1, pos[:, 1].max().item()+1)
            if (1<=i<3 and not is_txt) : # or ( i==3 and is_txt):  
                if self.method == 'yarn' and current_patches > self.base_patches:
                    ...
                    cos, sin = get_1d_rotary_pos_embed_(
                        **common_kwargs,
                        yarn=True,
                    ...
                    cos, sin = get_1d_rotary_pos_embed_(**common_kwargs, ntk_factor=ntk_factor)

                else: #base
                    cos, sin = get_1d_rotary_pos_embed_(**common_kwargs)
            else:
...
class Flux2PosEmbed_(nn.Module):
...
class Flux2Transformer2DModel(
    ...
    def forward(
        ...
        self.pos_embed.set_timestep(timestep)
        ...
        #print(encoder_hidden_states.device,encoder_hidden_states.dtype,self.context_embedder.weight.device,self.context_embedder.weight.dtype)
        if encoder_hidden_states.dtype!=self.context_embedder.weight.dtype:
            encoder_hidden_states = encoder_hidden_states.to(self.context_embedder.weight.dtype)
        ...
        if txt_ids.ndim == 3:
            txt_ids = txt_ids[0]
        #print(img_ids.shape,txt_ids.shape) #torch.Size([25600, 4]) torch.Size([512, 4])
        image_rotary_emb = self.pos_embed(img_ids,False)

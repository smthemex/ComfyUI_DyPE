...
class Flux2KleinPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    def __call__(
        ...
        if prompt is not None and isinstance(prompt, str):
            ...
            batch_size =1  #prompt_embeds.shape[0]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #print(latent_model_input.shape) #Size([1, 25600, 128]) 
        ...
        # latents_bn_mean = self.vae.running_mean.view(1, -1, 1, 1).to(latents.device, latents.dtype)
        # latents_bn_std = torch.sqrt(self.vae.running_var.view(1, -1, 1, 1) + self.vae.batch_norm_eps).to(
